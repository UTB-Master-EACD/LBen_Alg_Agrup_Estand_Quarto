---
title: "Metodología para la construccion de LBen de consumo absoluto utilizando algoritmos de agrupación"
author: "luisfflorezg"
format: 
  html:
...
En nuestro caso el formato csv - sep ";" es el que se está generando desde las fuentes primarias de información (editar en caso de utilizar otro dataset de prueba, hay 20 casos).
...
datatable( head (datos_preparados, 5) )
```
...
### Distribucion diaria de la variable consumo

Este paso analiza cómo varía el consumo energético a lo largo de los días de la semana (distribución diaria) y compara estas distribuciones entre distintos días. El objetivo es identificar patrones recurrentes en el consumo según el día de la semana y evaluar si estos se mantienen consistentes o presentan variaciones significativas.
...
graficar_promedio_por_dia <- function(datos) {
  # Agrupar por día de la semana y calcular el promedio de consumo
  datos_por_dia <- datos %>%
    group_by(dia_sem) %>%
    summarise(consumo_promedio = mean(consumo, na.rm = TRUE)) %>%
    arrange(factor(dia_sem, levels = c("lunes", "martes", "miércoles", "jueves", "viernes", "sábado", "domingo")))

  # Gráfico de consumo promedio por día de la semana
  ggplot(datos_por_dia, aes(x = dia_sem, y = consumo_promedio, fill = dia_sem)) +
    geom_col() +
    theme_minimal(base_size = 14) +
    labs(title = "Consumo Promedio por Día de la Semana",
         x = "Día de la Semana", y = "Consumo Promedio") +
    theme(plot.title = element_text(size = 14, hjust = 0.5)) +
    scale_fill_brewer(palette = "Set2")  # Paleta de colores opcional
}

# Ejemplo de uso
graficar_promedio_por_dia(datos_limpiados)
...
Sin embargo, si se detectaran problemas como clusters mal definidos o sensibilidad a valores atípicos, podría considerarse una validación adicional con **DBSCAN** y **HClust** para verificar la estabilidad de la segmentación (metodo seleccionable en el código).
...
(editar método y número de clusters)
...
### **Matriz de Franjas Horarias por Cluster (Definición de Perfiles de Consumo por Franja Horaria)**
...
### **Tabla de Franjas Horarias por Cluster (Definición de Perfiles de Consumo por Franja Horaria)**
...
datatable(estadisticas_residuos%>%
  mutate(across(where(is.numeric), ~ round(., 2))))
